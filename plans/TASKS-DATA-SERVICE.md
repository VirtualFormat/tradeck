...
- P1-1 真缺口永不收敛：周末被当「应有分区」但必返回空 → 每天固定 2-4 个周末假缺口刷屏淹没真缺口。
  修：detect 候选集剔周末；backfill 区分「该天无数据」与「补拉失败」。
- P1-2 检测失败静默：冷层不可达返回空 = 误报「无缺口」（生死线上最危险的静默方向）。
  修：detect 返回 (missing, detect_ok)，失败显式 logger.error 告警。
- P2-5 删 _partition_key 死代码。
- 未修（接受现状）：HK/US 假日不重合误报（需交易日历）；补拉窗口收紧 5-6 天（后续 tombstone 优化）。
