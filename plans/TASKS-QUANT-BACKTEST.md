...
- **Docker 守护进程在当前沙箱不可达**（colima socket 无权限），「容器内 fetch 实跑 + 真 data-api
  联调」未完成——需在 dev 环境 `docker compose --profile quant run --rm quant python -m app.cli fetch
  --symbols AAPL,600519.SH --start 2024-01-01` 复核一条，复核通过前不视为生产就绪。
- ex_factors 真实拉取未验（无付费 key），降级路径已验。

| 阶段 | 状态 | Review 问题 | 修复 | 验收结论 | 日期 |
|---|---|---|---|---|---|
| A 骨架与数据层 | ✅ 验收通过 | 见下 | 已处置 | **单元+端到端全绿**（Docker 沙箱不可达，容器实跑留 dev 环境复核） | 2026-08-25 |
| B 矩阵与回测引擎 | 未开始 | 无 | 无 | 无 | 无 |
| C 策略体系与 AI 生成 | 未开始 | 无 | 无 | 无 | 无 |
| D 选股与挖掘 | 未开始 | 无 | 无 | 无 | 无 |
