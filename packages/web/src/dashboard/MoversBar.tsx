  ...
  const rows = useMemo(() => {
    ...
    return stocks
      .map((t) => ({
        ...
        price: t.price,
        marketLabel: t.market ? MARKET_LABEL[t.market as Market] ?? t.source : t.source,
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (p: any) => {
          ...
          return `<b>${row.name}</b><br/>${row.symbol} · ${row.marketLabel}<br/>价格: ${row.price.toFixed(2)}<br/><span style="color:${color}">${value >= 0 ? '+' : ''}${value.toFixed(2)}%</span>`;
        },
      },
      series: [
        {
              ...
              <div key={row.symbol} className="grid grid-cols-[22px_1fr_64px] items-center gap-2 text-[11px]">
                ...
                <div className="min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="tab-nums shrink-0 text-muted">{row.price.toFixed(2)}</span>
                  </div>
                  ...
                  <div className="mt-0.5 truncate text-[9px] text-muted">{row.symbol} · {row.marketLabel}</div>
                </div>
