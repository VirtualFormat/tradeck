"""因子挖掘统计检验：Newey-West HAC t 值 / BH-FDR q 值 / DSR 通缩夏普。

照搬参照实现 tick-stock-panel backend/app/backtest/stats_v2.py 的数值口径，
运行时零新增第三方依赖（后端无 scipy/statsmodels），全部 numpy 手写；
数值精度由参照 tests/test_stats_v2.py 的黄金参考向量锁定（误差 < 1e-6）。
...
- DSR（Deflated Sharpe Ratio，Bailey & Lopez de Prado 2014）用于多重试验校正后的
  夏普显著性；期望最大夏普 EM = sqrt(V[SR]) * ((1-gamma)Phi^-1(1-1/N) + gamma Phi^-1(1-1/(Ne)))，
  其中 gamma 为欧拉-马歇罗尼常数。
"""
...
def newey_west_t(values, lag: int) -> tuple[float, float, float] | None:
    ...
    centered = array - mean
    # 长方差 S = gamma0 + 2 * sum_l w_l * gamma_l, w_l = 1 - l/(lag+1)（Bartlett）
    gamma = [
        float(np.dot(centered[: n - lag_i], centered[lag_i:]) / n)
...
def expected_max_sharpe(n_trials: int, variance_sharpes: float) -> float:
    ...
    return math.sqrt(variance_sharpes) * (
...
def deflated_sharpe_psr(
    ...
    if denominator <= 0:
        return None
    statistic = (
        (sharpe - expected_max_sharpe) * math.sqrt(n_obs - 1) / math.sqrt(denominator)
    )
    return 0.5 * (1.0 + math.erf(statistic / math.sqrt(2.0)))


def daily_ic_stats(
    ics: list[float], lag: int
) -> tuple[float | None, float | None, float | None]:
    """逐日 IC 序列的 (NW t 值, 双侧 p 值, 朴素 t 值)；任一退化返回对应 None。"""
    nw = newey_west_t(ics, lag)
    t = nw[0] if nw is not None else None
    p = normal_two_sided_p(t) if t is not None else None
    return t, p, naive_t(ics)
